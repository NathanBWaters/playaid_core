...
def vis_animations(
    ...
    model = CNNActionDetector.load_from_checkpoint(
        os.path.join(SAVED_ACTION_MODELS, "four-chars-aug-4.ckpt"),
        # os.path.join(SAVED_ACTION_MODELS, "byleth-and-diddy-july-24.ckpt"),
        char_subset=char_subset,
        actions=actions,
    ...
    for i in range(total):
        ...
        frame_delta = data['frame_delta']

        labels.append(int(action_label[center]))
        ...
        gt_label = actions[int(action_id[center])]
        is_accurate = gt_label == predicted_action
        caption = f"{'✅' if is_accurate else '❌'} "
        caption += f"Pred: {predicted_action} {confidence:.2f}% | {frame_delta} delta"
        if not is_accurate:
            caption += f" | GT: {gt_label}"
            caption += f" {data['frame_paths'][center]}"
        num_correct += is_accurate
        confidence_list.append(confidence)

        captions = [caption if i == center else " " for i in range(seq)]
